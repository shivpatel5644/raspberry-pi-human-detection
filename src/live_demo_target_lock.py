...
print("Target lock demo started. Press q to quit.")
...
try:
    while True:
        ...
        if person_candidates:
            if locked_target_center is None:
                ...
                if dist <= LOCK_DISTANCE_THRESHOLD:
                    ...
                    # Too far from last target position, count as potential loss
                    lost_frames += 1
                    lock_status = f"SEARCHING ({lost_frames})"

                    if lost_frames > MAX_LOST_FRAMES:
                        # Re-lock to largest person
                        best_person = max(person_candidates, key=lambda p: p["area"])
                        locked_target_center = best_person["center"]
                        lost_frames = 0
        ...
        cv2.imshow("YOLOv8 Target Lock Demo", img)
    ...
    print("Target lock demo stopped.")
